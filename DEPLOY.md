...
2. In the dashboard open **SQL Editor → New query**, paste the whole of [`supabase/schema.sql`](supabase/schema.sql) and click **Run**.
   This creates three tables (`tests`, `reading_attempts`, `writing_submissions`) with Row Level Security **on and no public policies**, so nobody can read your answer keys with the public anon key.
3. Open **Project Settings → API** and copy:
   - **Project URL** → `SUPABASE_URL`
   - **service_role** secret → `SUPABASE_SERVICE_ROLE_KEY` (never put this in frontend code or in git)
4. Upload the tests:
...
3. When prompted, fill in `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `ANTHROPIC_API_KEY` and `CONTACT_EMAIL`. `ADMIN_TOKEN` is generated for you; copy it from the Environment tab if you need the admin endpoints.
